...
st.title('Intertextuelle Netzwerke in deutschsprachigen Orgelpredigten der frühen Neuzeit')
...
3. Welche textuellen Ähnlichkeiten im Predigttext werden maschinell in einem Vergleichskorpus erkannt, wie unterscheiden sich hier Identifikationen basierend auf string-Vergleich und basierend auf einer Vektorrepräsentation des Textes? Wie überschneiden sie sich untereinander und mit den manuell markierten Daten?""")
...
Grundsätzlich ist zu beachten, dass, während die zu den Predigten gelieferten Metadaten auf den entsprechenden Datensätzen des Projektes basieren, die Angaben zu Zitationen in Predigten _allein auf im Fließtext markierten Passagen_ beruhen, also nicht auf Marginalien oder Erwähnungen in Fußnoten oder Einführungstexten. Aufgrund dessen können sich die hier präsentierten Daten teils anders ausnehmen als in der Darstellung im Orgelpredigt-Portal.""")
    ...
    st.markdown("""Die Startseite vermittelt einen generellen Überblick. Auf der Seite 'Orgelpredigt Analyse' können einzelne Predigten genauer inspiziert und im Volltext angezeigt werden. Über 'Orgelpredigt Vergleich' können zwei oder mehr Predigten gemeinsam auf Überschneidungen hin untersucht werden. Die Seite 'Quellen Analyse' erlaubt die Untersuchung einzelner Quellen auf Verteilung im Korpus, zitierte Passagen und Korrelationen mit anderen Quellen hin.
